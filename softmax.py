...
def compute_probabilities(X, theta, temp_parameter):
    ...
    c = dot_products.max(axis=0)                    # axis = 0 => columns
    exponents = dot_products - c                    # c = max of columns
    exponentiated = np.exp(exponents)
    col_sums = exponentiated.sum(axis=0)
    return exponentiated / col_sums               # returns the probability
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    Returns
        the cost value (scalar) Cost function = J(theta)
    """
    N = X.shape[0]
    probability = compute_probabilities(X, theta, temp_parameter)
    choice = np.choose(Y, probability)
    non_regulizing_cost = np.sum(np.log(choice))
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    non_regularized_gradient = np.dot(M-probability, X)
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    return 1-np.mean(np.remainder(assigned_labels,3)==Y)                    #similar to compute_test_error(X,Y,theta,temp_parameter)

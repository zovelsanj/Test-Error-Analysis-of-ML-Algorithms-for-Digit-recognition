...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    """
    # STEP1 : first center data using the feature_means
    # STEP2 : Return the projection of the centered dataset
    #       on the first n_components principal components.
    #       This should be an array with dimensions: n x n_components.
    # these principal components = first n_components columns of the eigenvectors returned by principal_components().
    # Note that each eigenvector is already be a unit-vector, so the projection may be done using matrix multiplication.
    #n_components = dimension of PCA feature matrix
    centered_data = X -feature_means
    projection = np.dot(centered_data, pcs[:,range(n_components)])      #projection => dot product
    return projection  
